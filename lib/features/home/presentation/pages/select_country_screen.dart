...
class _SelectCountryScreenState extends State<SelectCountryScreen> {
  ...
  Future<void> setUserCounty(String countryCode, String countryName) async {
    context.read<HomeBloc>().add(SetUserCountryCode(countryCode, countryName));
  }
    ...
    return Scaffold(
      ...
      body: BlocConsumer<HomeBloc, HomeState>(
        listener: (context, state) {
          if (state is CountrySaveLoading) {
            Log.debug("country save loading");
          }
          if (state is CountrySaveSuccess) {
            ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Country saved successfully!")));
            context.read<HomeBloc>().add(const GetTopArticles());
            context.pop();
          }
        },
                      ...
                      child: MainBtn(
                        onClick: () {
                          if (selectedCounty != null) {
                            setUserCounty(selectedCounty!.code ?? 'us',
                                selectedCounty!.name ?? 'United states');
                          }
                        },
